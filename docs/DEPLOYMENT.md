...
2. **Variáveis de ambiente**
   ...
   | `SUPABASE_STORAGE_BUCKET_LINKS` | Opcional | Nome do bucket no Supabase Storage para ícones dos links úteis. Se definido (com `SUPABASE_SERVICE_ROLE_KEY`), os ícones passam a ser salvos no Storage e a URL persiste após cada deploy. Caso contrário, os ícones ficam em `public/uploads/links` e só persistem se você versionar e enviar esses arquivos no Git. |
...
- [ ] **Ícones dos links úteis:** Se não usar Supabase Storage (`SUPABASE_STORAGE_BUCKET_LINKS`), os ícones em `public/uploads/links` só persistem se versionados no Git; após deploy limpo podem sumir. Para evitar isso, crie um bucket público no Supabase (ex.: "links") e defina `SUPABASE_STORAGE_BUCKET_LINKS=links`.
---
